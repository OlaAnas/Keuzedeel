...
<div class="container">
    <div class="content">
        ...
        <form method="POST" action="{{ route('keuzedelen.update', $keuzedeel->id) }}">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                <div>
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code', $keuzedeel->code) }}" required style="width: 100%; padding: 0.75rem; border: 1px solid {{ $errors->has('code') ? '#dc3545' : '#ddd' }}; border-radius: 4px; font-size: 1rem;">
                    @error('code')
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $keuzedeel->name) }}" required style="width: 100%; padding: 0.75rem; border: 1px solid {{ $errors->has('name') ? '#dc3545' : '#ddd' }}; border-radius: 4px; font-size: 1rem;">
                    @error('name')
                        <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <textarea id="description" name="description" rows="4" required style="width: 100%; padding: 0.75rem; border: 1px solid {{ $errors->has('description') ? '#dc3545' : '#ddd' }}; border-radius: 4px; font-size: 1rem; font-family: inherit;">{{ old('description', $keuzedeel->description) }}</textarea>
                @error('description')
                    <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                <div>
                    ...
                    <input type="number" id="min_students" name="min_students" value="{{ old('min_students', $keuzedeel->min_students) }}" min="1" required style="width: 100%; padding: 0.75rem; border: 1px solid {{ $errors->has('min_students') ? '#dc3545' : '#ddd' }}; border-radius: 4px; font-size: 1rem;">
                    @error('min_students')
                        <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="max_students" name="max_students" value="{{ old('max_students', $keuzedeel->max_students) }}" min="1" required style="width: 100%; padding: 0.75rem; border: 1px solid {{ $errors->has('max_students') ? '#dc3545' : '#ddd' }}; border-radius: 4px; font-size: 1rem;">
                    @error('max_students')
                        <span style="color: #dc3545; font-size: 0.85rem;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="teacher_id" name="teacher_id" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
                        ...
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('teacher_id', $keuzedeel->teacher_id) == $teacher->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="study_id" name="study_id" style="width: 100%; padding: 0.75rem; border: 1px solid #ddd; border-radius: 4px; font-size: 1rem;">
                        ...
                        @foreach($studies as $study)
                            <option value="{{ $study->id }}" {{ old('study_id', $keuzedeel->study_id) == $study->id ? 'selected' : '' }}>
                                {{ $study->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div style="margin-bottom: 2rem;">
                <label style="display: flex; align-items: center; gap: 0.5rem; font-weight: 500;">
                    <input type="checkbox" id="repeatable" name="repeatable" value="1" {{ old('repeatable', $keuzedeel->repeatable) ? 'checked' : '' }} style="width: auto;">
                    Repeatable
                </label>
            </div>
